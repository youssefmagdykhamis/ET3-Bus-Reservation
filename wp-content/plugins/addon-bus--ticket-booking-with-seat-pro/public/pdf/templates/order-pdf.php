...
$template   = isset( $atts['template'] ) ? $atts['template'] : $magepdf->get_option( 'magepdf_invoice_template', 'flat' );
...
if( empty( $order_id ) || $order_id == 0 ) {
    $magepdf->print_error( new WP_Error( 'invalid_data', __('Order ID missing!' ) ) );
    return;
}

$all_templates  = $magepdf->get_templates();

// print_r($all_templates);
...
if( empty( $template_dir ) || empty( $stylesheet_dir ) ) {
    ...
    $magepdf->print_error( new WP_Error( 'invalid_data', sprintf( __( 'Template file missing : <b>%s</b>' ), ucwords( $template ) ) ) );
    return;
}
