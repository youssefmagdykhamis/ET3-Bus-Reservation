...
if(isset($_POST['add-to-cart-admin'])){
 ...
 $fare          = sanitize_text_field($_POST['bus_fare']);
...
$count_user = count($wbtm_user_name);
  ...
  $user ="";
      ...
      $bus_fare              = $bus_fare + $extra_bag_price;
...
// if($check_before_add==0){
  ...
  $add_ticket = $wbtmmain->create_bus_passenger($order_id,$bus_id,$user_id,$start,$next_stops,$end,$b_time,$j_time,$_seats,$fare,$j_date,$add_datetime,$user_name,$user_phone,$user_gender,$user_address,$wbtm_extra_bag_qty,$usr_inf,$counter,2 );
   
// if($add_ticket){
      $download_url = $magepdf->get_invoice_ajax_url( array( 'order_id' => $order_id ) );
      ?>
<div id="message" class="updated notice notice-success is-dismissible"><p><?php _e('Seat Booked Successfully','addon-bus--ticket-booking-with-seat-pro') ?>. <a class="wbtm-button" href="<?php echo $download_url; ?>" style="background: green;color: #fff;padding: 5px 20px;font-size: 14px;font-style: normal;text-decoration: none;margin: 0 auto;width: 200px;display: inline;text-align: center;"><?php _e('Download Ticket','addon-bus--ticket-booking-with-seat-pro'); ?></a></p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button></div>
      <?php
    // }
    // }
    }
...
?>
            <form action="" method='post'>
                    ...
                    <div class="leaving-list">
                        ...
                        <div class="selected_route">
                            <?php printf( '<span>%s <i class="fa fa-long-arrow-right"></i> %s<span>', $start, $end ); ?>
                             ...
                             <input type="hidden" name="bus_fare" value="<?php echo $wbtmmain->wbtm_get_bus_price($start,$end, $price_arr); ?>">
                        </div>
                    </div>                    
                    <div class="leaving-list">
                        <h6>Date</h6>
                ...
                <div class="seat-selected-list-fare">
                    <table class="selected-seat-list<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>">
                        ...
                        <tr>
                            <td align="center">Total <span id='total_seat<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>_booked'></span><input type="hidden" value="" id="tq<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>" name='total_seat' class="number"/></td>
                            
                            <td align="center"><input type="hidden" value="" id="tfi<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>" class="number"/><span id="totalFare<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>"></span></td><td></td>
                        </tr>
                    </table>
                    ...
                    <button id='bus-booking-btn<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>' type="submit" name="add-to-cart-admin" value="<?php echo esc_attr(get_the_id()); ?>" class="single_add_to_cart_button button alt btn-mep-event-cart">Book Now</button>
                </div>
            </form>
        </div>
	</div>
