...
<div class="filter-fullwidth">
    <?php echo st()->load_template('layouts/elementor/hotel/elements/toolbar2', '', array('style' => $style, 'format' => $format, 'layout' => $layout)); ?>
    <div id="modern-search-result" class="modern-search-result" data-layout="1">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/elementor/hotel/loop/normal', $style, array('fullwidth' => true));
            }
        }else{
