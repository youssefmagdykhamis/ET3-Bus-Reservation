...
<div class="col-sm-12 col-md-9 col-lg-9">
    <?php echo st()->load_template('layouts/elementor/hotel/elements/toolbar', '', array('style' => $style, 'format' => $format, 'layout' => $layout, 'service_text' => __('New tour', 'traveler'), 'post_type' => 'st_tours')); ?>
    <div id="modern-search-result" class="modern-search-result" data-layout="1">
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/elementor/tour/loop/normal-' . esc_attr($style), '',array('item_row'=> 3));
            }
        } else {
