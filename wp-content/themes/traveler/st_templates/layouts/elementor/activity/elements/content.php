...
<div class="col-lg-9 col-md-9">
    <?php echo st()->load_template('layouts/elementor/hotel/elements/toolbar', '', array('style' => $style, 'format' => $format, 'layout' => $layout, 'service_text' => __('New activity', 'traveler'), 'post_type' => 'st_activity')); ?>
    <div id="modern-search-result" class="modern-search-result" data-layout="1">
        ...
        if($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                echo st()->load_template('layouts/elementor/activity/loop/normal-' . esc_attr($style), '',array('item_row'=> 3));
            }
        }else{
