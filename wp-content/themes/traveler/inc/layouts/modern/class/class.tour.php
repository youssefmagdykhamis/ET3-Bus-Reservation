...
if (!class_exists('STTour')) {
    class STTour extends TravelerObject {
                ...
                if ($max_number > 0) {
                    ...
                    if($tour_price_by == 'fixed'){
                        if(!empty($result) && !empty(trim($starttime))){
                            wp_send_json(array('message' =>sprintf(__('This tour is not available.', 'traveler')),'price_from' => $price_from));
                        }
                    }
        ...
        public function st_filter_tour_ajax_location() {
            ...
            <?php $ajax_filter_content = ob_get_contents();
            ob_clean();
            ob_end_flush();
            ob_start();
        ...
        public function st_filter_tour_ajax() {
            ...
            $top_search = STInput::get('top_search');
            if ($top_search){
                $arr_data = array('top_search' => true ,'item_row'=> 4);
            }
        ...
        function do_add_to_cart() {
            ...
            if ($pass_validate) {
                ...
                if ($pass_validate) {
                    STCart::add_cart($item_id, $number, $total_price + $extra_price + $package_hotel_price + $package_activity_price + $package_car_price + $package_flight_price, $data);
                }
            }
