            ...
            static function _check_room_only_available( $room_id = '', $check_in = '', $check_out = '', $number_r = 0, $order_item_id = '' )
            {
                ...
                if ( is_array( $result ) && count( $result ) ) {
                    ...
                    if ( ( is_array( $list_date_fist_half_day ) and count( $list_date_fist_half_day ) ) and ( is_array( $list_date_last_half_day ) and count( $list_date_last_half_day ) ) ){
                        if(
                            (strtotime($list_date_fist_half_day[0]) > strtotime($list_date_last_half_day[0]))
                            &&(count( $list_date_fist_half_day ) > 0 && count( $list_date_last_half_day ) > 0)
                             ){
                            return true;
                        } else {
                            return false;
            ...
            public function _get_availability_hotel_room()
            {
                ...
                if ( !empty( $room_available ) ) {
                    foreach ( $room_available as $kk => $vv ) {
                        if($price_by_per_person === 'on'){
                            $ev = __('Adult: ', 'traveler') . TravelHelper::format_money(st_apply_discount( $vv[ 'adult_price' ], $discount_type, $discount )) . '<br/>';
                            $ev .= __('Child: ', 'traveler') . TravelHelper::format_money(st_apply_discount( $vv[ 'child_price' ], $discount_type, $discount )) . '<br/>';
